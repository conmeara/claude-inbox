...
`;
    ...
    async callClaude(prompt, retries = 0, onProgress) {
        ...
        try {
            let response = '';
            // Include system prompt in the user prompt
    ...
    buildBatchSummaryPrompt(emails) {
...
... and so on`;
    ...
    parseBatchSummaries(emails, response) {
        const summaries = [];
    ...
    async summarizeEmailsIndividually(emails) {
        const summaries = [];
        for (let i = 0; i < emails.length; i++) {
...
Draft a polite email asking for this specific clarification while acknowledging their original message.`;
    ...
    async improveEmailDraft(originalDraft, userFeedback, email, onProgress) {
...
Provide an improved draft that addresses the feedback while maintaining professionalism.`;
    ...
    async *streamBatchProcess(emails) {
